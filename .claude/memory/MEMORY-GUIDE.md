...
แม้ memory จะโตเป็นหลายร้อยไฟล์ Pixel ต้องทำตามคู่มือนี้ทุกครั้งที่จัดการ memory

## หลักการ (ทำไมไม่บวม)
- **เฉพาะ `MEMORY.md` ที่โหลดทุก session** (200 บรรทัด/25KB แรก) → คุมให้ **≤150 บรรทัด**
- Topic files **ไม่โหลดตอนเริ่ม** — Pixel เปิดอ่าน on-demand เมื่อ description ใน index ชี้ว่าเกี่ยว
...
| `tools/` | spec ต่อ **tool** (1 ไฟล์ = 1 route = 1 folder ใน `components/`) — สร้างด้วย `/new-tool` |
| `log/` | working log/progress — เก็บล่าสุดที่ active, เก่าย้าย `_archive/` |
| `reference/` | pattern/สูตร/gotcha ที่หยิบใช้ซ้ำ (ไม่ใช่ decision) |
| `_archive/` | library/cold storage — **ไม่ลิสต์ใน MEMORY.md** มี `INDEX.md` เป็น catalog |

> 💡 `tools/` คือ retheme ของ `modules/` ตามโดเมนโปรเจคนี้ (โดเมน = tool ทำ asset เกม) — slug ต้องตรงกับชื่อ route

## Frontmatter มาตรฐาน (ทุก topic file)
```yaml
---
name: <slug-kebab-case>
description: <1 บรรทัด ช่วย Pixel ตัดสินใจ recall — บอกว่า "อ่านเมื่อ...">
metadata:
  type: persona | overview | decision | module | convention | log | reference
...
- `description` สำคัญสุด — recall ไม่ใช่ semantic อัตโนมัติ Pixel เลือกเปิดจาก description ใน index
...
- Pixel ต้อง **เตือนพี่** เมื่อ index ใกล้เต็ม (`/memory-status` ช่วยตรวจ)
